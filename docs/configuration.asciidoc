...
We recommend the method of specifying options in a `config/elastic_apm.yml` file:
...
When using Sinatra and Rack, configure when starting
the agent:
...
The API key must be created using the APM server command line tool. Please see the APM server
documentation for details on how to create the API key. Please note that this feature is experimental in the
...
If the limit is hit, increase the agent's
<<config-pool-size,worker pool size>> or the agent is having trouble
connecting to APM Server. The <<config-log-path,logs>> should help to troubleshoot.
...
By setting this to `False`, the tracking is disabled, which can reduce the overhead of the agent.
...
Sometimes it is necessary to sanitize the data sent to Elastic APM to remove sensitive data.
...
The name of your service. This is used to group the errors and transactions of your service and is the 
...
Your service name must only contain characters from the ASCII
...
Elastic APM patches `Kernel#require` to auto-detect and instrument supported third party libraries. However, it may conflict with some libraries.
